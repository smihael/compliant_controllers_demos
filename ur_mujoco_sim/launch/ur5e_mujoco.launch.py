"""UR5E arm using the ros-controls MuJoCo hardware plugin."""
...
def setup(context):
    share = Path(get_package_share_directory('ur_mujoco_sim'))
    model_dir = Path(LaunchConfiguration('model_dir').perform(context))
    robot = ET.parse(model_dir / 'ur5e.urdf').getroot()
    control = ET.SubElement(robot, 'ros2_control', name='UrMujocoSystem', type='system')
    ...
    return [
        ...
        Node(package='mujoco_ros2_control', executable='ros2_control_node',
             parameters=[str(share / 'config/ur5e_controllers.yaml'),
                 {'use_sim_time': False, 'robot_description': description}],
             remappings=[('~/robot_description', '/robot_description')],
             output='screen', on_exit=Shutdown()),
        Node(package='ur_mujoco_sim', executable='start_controllers.py',
...
def generate_launch_description():
    share = get_package_share_directory('ur_mujoco_sim')
