#!/usr/bin/env python3
"""Generate matching UR5E URDF/MJCF from ur_description (arm without hand)."""
import argparse
import copy
...
HOME = [0.0, -1.5707963268, 1.5707963268, -1.5707963268, -1.5707963268, 0.0]
    ...
    for link in robot.findall('link'):
        ...
        for visual_index, visual in enumerate(link.findall('visual')):
            ...
            if mesh_element is None or body is None:
                raise ValueError('Expected a mesh visual on a retained UR5E body')
...
def generate(output):
    ...
    robot = ET.fromstring(xacro.process_file(
        ...
        mappings={'name': 'ur5e', 'ur_type': 'ur5e', 'use_fake_hardware': 'true'},
    ).toxml())
    ...
    ET.ElementTree(robot).write(output / 'ur5e.urdf', encoding='unicode')
    ...
    # small Cartesian step. These are demo dynamics, not identified UR5E values.
    ...
    assert model.nq == model.nv == model.nu == len(JOINTS), 'Expected a 6-axis UR5E arm'
    ...
    source.unlink()
    print(f'Validated 6-joint UR5E model: {scene_path}')
