#!/usr/bin/env python3
"""Generate matching FR3 URDF/MJCF from franka_description (arm without hand)."""
import argparse
import copy
    ...
    for link in robot.findall('link'):
        ...
        for visual_index, visual in enumerate(link.findall('visual')):
            ...
            if mesh_element is None or body is None:
                raise ValueError('Expected a mesh visual on a retained FR3 body')
            ...
            for part_index, node in enumerate(imported.graph.nodes_geometry):
                ...
                color = np.asarray(material.main_color, dtype=float) / 255.0
...
def generate(output):
    ...
    robot = ET.fromstring(xacro.process_file(
        str(description / 'robots/fr3/fr3.urdf.xacro'),
        mappings={'hand': 'false', 'ros2_control': 'false', 'gazebo': 'false',
                  'with_sc': 'false', 'safety_distance': '0.0'},
    ).toxml())
    ...
    ET.ElementTree(robot).write(output / 'fr3.urdf', encoding='unicode')
    ...
    # small Cartesian step. These are demo dynamics, not identified FR3 values.
    ...
    assert model.nq == model.nv == model.nu == 7, 'Expected a seven-axis FR3 arm'
    for index in range(7):
        assert model.joint(index).name == f'fr3_joint{index + 1}'
    data = mujoco.MjData(model)
    data.qpos[:] = HOME
    mujoco.mj_forward(model, data)
    assert data.ncon == 0, 'Initial pose must be free of contacts'
    source.unlink()
    print(f'Validated seven-joint FR3 model: {scene_path}')
