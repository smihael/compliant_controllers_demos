#!/usr/bin/env python3
"""Generate matching Rizon 4s URDF/MJCF from flexiv_description (arm without hand)."""
import argparse
import copy
...
def add_visuals(robot, root, output):
    """Bake visual mesh transforms and export each material as a visual-only OBJ."""
    ...
    for link in robot.findall('link'):
        ...
        for visual_index, visual in enumerate(link.findall('visual')):
            ...
            if mesh_element is None or body is None:
                raise ValueError('Expected a mesh visual on a retained Rizon 4s body')
...
def generate(output):
    ...
    # small Cartesian step. These are demo dynamics, not identified Rizon 4s values.
    ...
    assert model.nq == model.nv == model.nu == len(JOINTS), 'Expected a 7-axis Rizon 4s arm'
    ...
    # Cross-check the dynamics used by the controller against the physics model.
    controller_model = pinocchio.buildModelFromUrdf(str(output / 'rizon4s.urdf'))
    controller_data = controller_model.createData()
    gravity = pinocchio.computeGeneralizedGravity(controller_model, controller_data, np.asarray(HOME))
    np.testing.assert_allclose(data.qfrc_bias, gravity, atol=1e-7)
    for link in robot.findall('link'):
        mass = link.find('inertial/mass')
        if mass is not None:
            np.testing.assert_allclose(model.body(link.get('name')).mass[0], float(mass.get('value')), atol=1e-9)

    source.unlink()
    print(f'Validated 7-joint Rizon 4s model: {scene_path}')
