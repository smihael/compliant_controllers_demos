"""onrobot and robotiq tools on the shared franka_real arm bringup."""
import json
import os
import tempfile
...
def configure_description(context):
    mappings = {name: LaunchConfiguration(name).perform(context) for name in (
        'xyz_onrobot', 'rpy_onrobot', 'xyz_robotiq', 'rpy_robotiq', 'com_port',
    )}
    ...
    return LaunchDescription(arguments + [
        ...
        IncludeLaunchDescription(PythonLaunchDescriptionSource(PathJoinSubstitution([
            ...
            'description_file': PathJoinSubstitution([
                FindPackageShare('franka_onrobot_robotiq'), 'urdf', 'fr3_onrobot_ft_robotiq_2f_85.urdf.xacro',
            ]),
            'load_gripper': 'false',
