...
Usage:
    ros2 launch lbr_gz_sim lbr_gazebo.launch.py \
        model:=iiwa14 \
...
def include_gazebo(context, *args, **kwargs):
    """Start Gazebo with or without its GUI."""
    show_gui = LaunchConfiguration("show_gazebo_gui").perform(context).lower() in (
        "true", "1", "yes"
    )
    gz_args = "-r empty.sdf" if show_gui else "-r -s --headless-rendering empty.sdf"
    return [
...
def generate_robot_description_with_effort(context, *args, **kwargs):
    ...
    # Match the small-command MuJoCo demo while retaining viscous damping.
    joint_friction = LaunchConfiguration("joint_friction").perform(context)
    for joint in root.findall('joint'):
        if joint.get('type') in ('revolute', 'continuous'):
            dynamics = joint.find('dynamics')
            if dynamics is not None:
                dynamics.set('friction', joint_friction)
...
def generate_launch_description() -> LaunchDescription:
    ...
    ld.add_action(DeclareLaunchArgument("joint_friction", default_value="0.0",
                                        description="Simulated joint dry friction in Nm; upstream uses 0.1"))
    ld.add_action(DeclareLaunchArgument("impl_library", default_value="libcartesian_impedance_impl.so"))
    ld.add_action(DeclareLaunchArgument("show_gazebo_gui", default_value="false"))
    ld.add_action(DeclareLaunchArgument("show_rviz", default_value="false"))
    ...
    # Gazebo
    ld.add_action(OpaqueFunction(function=include_gazebo))
